...
public class WeightedGraph<V> extends UnweightedGraph<V>
{
    ...
    public MST getMSTUsingKruskalAlgorithm(int startVertex)
    {
        int[] parent = new int[getSize()];
        parent[startVertex] = -1;

        int[] id = new int[getSize()];
        for (int i = 0; i < getSize(); i++)
            id[i] = i;
        ...
        for (int i = 0; i < getSize(); i++)
            neighbors.get(i).forEach(e -> edges.add((WeightedEdge) e));
        ...
        for (WeightedEdge edge : edges)
        {
            ...
            int oldParent = id[edge.u], newParent = id[edge.v];
            for (int i = 0; i < getSize(); i++)
            {
                if (id[i] == oldParent)
                    id[i] = newParent;
