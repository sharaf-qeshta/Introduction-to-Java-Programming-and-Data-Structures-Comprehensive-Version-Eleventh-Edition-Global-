...
public class WeightedGraph<V> extends UnweightedGraph<V>
{
    ...
    public List<Integer> getShortestHamiltonianCycle() throws Exception
    {
        ...
        for (int i = 0; i < getSize(); i++)
        {
            ...
            getShortestHamiltonianCycle(new ArrayList<>(), i, cycle, i, 0);

            if (cycle.size() > 0)
            {
                double currentCycleCost = 0;
                for (int j = 0; j < cycle.size()-1; j++)
                    currentCycleCost += getWeight(cycle.get(j), cycle.get(j+1));


                if (currentCycleCost < cost)
                {
                    cost = currentCycleCost;
                    list.clear();
                    list.addAll(cycle);
                }
            }
    ...
    private void getShortestHamiltonianCycle(List<Integer> visited,
                                     int currentNode, List<Integer> cycle,
                                     int start, double cost) throws Exception
    {
        if (visited.size() == getSize() && currentNode == start)
        {
            ...
            for (int i = 0; i < cycle.size()-1; i++)
                previousCost += getWeight(cycle.get(i), cycle.get(i+1));

            if (cost >= previousCost && previousCost != 0)
                return;
        ...
        for (int node: getNeighbors(currentNode))
            getShortestHamiltonianCycle
                    (new ArrayList<>(visited), node, cycle, start,
                            cost + getWeight(currentNode, node));
    }
}
