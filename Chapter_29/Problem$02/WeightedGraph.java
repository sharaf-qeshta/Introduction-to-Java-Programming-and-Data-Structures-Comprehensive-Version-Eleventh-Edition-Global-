...
public class WeightedGraph<V> extends UnweightedGraph<V>
{
    ...
    private void createWeightedGraph(List<V> vertices, List<WeightedEdge> edges)
    {
        ...
        for (WeightedEdge edge : edges)
            neighbors[edge.u][edge.v] = edge; // Add an edge into the matrix
    }
    ...
    {
        for (Edge edge : neighbors[u])
        {
            if (edge != null && edge.v == v)
            {
    ...
    public MST getMinimumSpanningTree(int startingVertex)
    {
        ...
        while (T.size() < getSize())
        {
            ...
            {
                if (e != null && !T.contains(e.v) && cost[e.v] > ((WeightedEdge) e).weight)
                {
